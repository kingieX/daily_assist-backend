...
const staffAlertSchema: OpenAPIV3.SchemaObject = {
  ...
  properties: {
    id: { type: 'string' },
    type: { type: 'string', enum: ['new_visit', 'visit_reminder', 'announcement', 'message', 'system'] },
    title: { type: 'string' },
    message: { type: 'string' },
...
export const visitPaths: OpenAPIV3.PathsObject = {
  ...
  '/staff/alerts': {
    get: {
      ...
      description: 'Returns alerts from the shared notifications table for the authenticated staff member. Visit assignment/reassignment/cancellation, announcement, message, and system alerts are generated by the same worker-driven pipeline used by admin dashboard alerts rather than computed ad hoc on read.',
      security: staffSecurity,
