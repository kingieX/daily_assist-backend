...
const markNotificationRead = asyncHandler(async (req: Request, res: Response) => {
  const user = currentUser(req);
  const result = await notificationsService.markNotificationRead(req.params.id as string, user.id);
...
const deleteNotification = asyncHandler(async (req: Request, res: Response) => {
  const user = currentUser(req);
  const result = await notificationsService.deleteStaffNotification(req.params.id as string, user.id);
...
export const staffNotificationsController = {
  listNotifications,
